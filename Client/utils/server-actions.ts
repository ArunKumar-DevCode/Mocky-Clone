...
export async function fetchMocks(): Promise<MockFormData[]> {
  ...
  try {
    const response = await axios.get("https://mocky-clone-gca8.vercel.app/api/mocks/all", {
...
export async function handleDeleteMock(id: string | undefined) {
  ...
  try {
    const res = await fetch(`https://mocky-clone-gca8.vercel.app/api/mocks/delete/${id}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
...
export async function getMockById(id: string) {
  ...
  const response = await fetch(
    `https://mocky-clone-gca8.vercel.app/api/mocks/response/${id}`,
    {
      cache: "no-store",
      next: { tags: ["mocks"] },
      headers: {
